...
{
    Task<PaginatedResult<LeadListDto>> GetLeadsAsync(int userId, PaginationParams pagination, string? search, string? stage, string? source, int? filterUserId = null);
    ...
    Task<List<LeadListDto>> GetLeadsByStageAsync(int userId, int? filterUserId = null);
